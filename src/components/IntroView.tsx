...
export default function IntroView({ onStart, onViewLeaderboard }: IntroViewProps) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -15 }}
      className="flex flex-col items-center justify-between min-h-[500px] py-6 text-center select-none"
      id="intro-view-container"
    >
      {/* Badge thương hiệu */}
      <div 
        className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#f4e7c9] text-[#5d4037] text-xs font-black rounded-none border-2 border-[#5d4037] shadow-[2px_2px_0px_0px_rgba(62,39,35,0.45)] mb-4 font-pixel"
        id="fe-badge"
      >
      ...
      <div className="flex flex-col items-center my-auto px-4" id="intro-main-content">
        <motion.div 
          className="relative flex items-center justify-center w-28 h-28 bg-white border-4 border-[#5d4037] rounded-none shadow-[4px_4px_0px_0px_rgba(62,39,35,0.35)] mb-6"
          animate={{ y: [0, -6, 0] }}
        ...
        {/* Tiêu đề chính */}
        <h1 className="text-5xl font-extrabold tracking-tight text-[#5d4037] mb-1 uppercase font-pixel drop-shadow-[2px_2px_0px_rgba(62,39,35,0.15)]" id="app-title">
          POST-A-TREE
        </h1>
        <p className="text-base font-bold tracking-wider text-accent-orange uppercase mb-4 font-pixel" id="app-sub-title">
          Chiến dịch trồng cây POST-A-TREE Mùa 6
        </p>

        {/* Nội dung giới thiệu ý nghĩa */}
        <div className="max-w-md bg-[#f4e7c9] rounded-none p-5 border-4 border-[#5d4037] shadow-[4px_4px_0px_0px_rgba(62,39,35,0.45)] mb-6 relative overflow-hidden" id="intro-description-card">
          <div className="absolute top-0 right-0 w-12 h-12 bg-[#81c784]/25 rounded-full blur-lg" />
          <p className="text-[#3e2723] text-sm md:text-base leading-relaxed font-bold font-pixel">
      ...
      {/* Hành động */}
      <div className="w-full max-w-sm px-4 space-y-4 mt-auto" id="intro-actions">
        <button
          type="button"
          onClick={onStart}
          className="w-full bg-[#d84315] hover:bg-[#bf360c] active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_rgba(62,39,35,0.45)] text-white font-bold py-3.5 px-6 rounded-none shadow-[4px_4px_0px_0px_rgba(62,39,35,0.45)] text-lg transition-all border-4 border-[#3e2723] pointer-events-auto cursor-pointer uppercase tracking-wider font-pixel"
          id="btn-start-game"
        >
          Bắt đầu chơi ngay
        </button>
